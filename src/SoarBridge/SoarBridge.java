...
public class SoarBridge {
    ...
    private void prepareInputLink() {
        ...
        try {
            if (agent != null) {
                ...
                for (Thing t : new ArrayList<Thing>(thingsList)) {
                ...
                c.getLeaflets().get(0).getWhatToCollect().forEach((jewel, quantity) -> {
                    Identifier leafletJewelId = CreateIdWME(creatureLeaflet, (String) jewel);
                    CreateIntegerWME(leafletJewelId, "QUANTITY", (int) quantity);            
                });
            }
        } catch (Exception e) {
            ...
            if (agent != null) {
                ...
                for (Wme com : Commands) {
                    ...
                    switch (commandType) {
                        ...
                        case STOP:
                            commandList.add(new Command(Command.CommandType.STOP));
                            break;
     ...
     *
     * @param soarCommandStop Soar Stop Command Structure
     */
    private void processStopCommand(CommandStop soarCommandStop) throws CommandExecException {
        if (soarCommandStop!= null) {
            ...
            logger.severe("Error processing processStopCommand");
        }
    }
